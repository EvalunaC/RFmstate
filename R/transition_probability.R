...
#'   Names must be \code{"from->to"}; each frame contains sortable
#'   \code{time} and nondecreasing \code{hazard} (or \code{cum_hazard}).
#'   Duplicate times retain their last supplied value. An explicit duration
#'   zero must have cumulative hazard zero within tolerance; when the first
#'   supplied time is positive, \code{(0, 0)} is prepended.
...
#' @param max_grid_points Maximum number of internal grid points, including
...
#' @param check_grid Whether to require grid-refinement convergence. Keep
#'   \code{TRUE} for reported analyses. It must be one nonmissing logical;
#'   \code{FALSE} warns and records an unchecked result.
#' @param prob_tol Positive tolerance for probability bounds and unit mass.
...
#'   alias \code{P} are arrays ordered selected starting state by occupied
#'   state by elapsed time; the starting-state dimension has length one.
#'   \code{state_occ} is the selected starting-state slice. The
#'   object also records \code{time}, structure, fresh-entry conditioning,
...
#' approximations. A positive cumulative hazard at duration zero is rejected
#' because the data contract permits no instantaneous fresh-entry transition.
...
#' not estimate hazards, accept left truncation or \code{s > 0}, condition on
...
compute_trans_prob <- function(cum_hazards, structure, s = 0, times = NULL,
  ...
  .validate_scalar_logical(check_grid, "check_grid")
  ...
  structure(
    list(
      ...
      grid_checked = isTRUE(check_grid),
      ...
      hazard_roundoff_corrections = solved$hazard_corrections +
        (attr(curves, "roundoff_corrections") %||% 0L),
      prob_tol = prob_tol
    ),
    class = "trans_prob"
  )
...
.validate_hazard_curves <- function(cum_hazards, structure) {
  ...
  for (edge in expected) {
    ...
    ordering <- order(tt, seq_along(tt))
    tt <- tt[ordering]
    aa <- aa[ordering]
    if (anyDuplicated(tt)) {
    ...
    if (tt[1L] == 0) {
      if (aa[1L] > hazard_tol_abs) {
        stop("Cumulative hazard at duration zero must be zero for edge '",
             edge, "'; observed H(0) = ", format(aa[1L]), ".")
      }
      if (aa[1L] > 0) {
        aa[1L] <- 0
  ...
  attr(curves, "roundoff_corrections") <- corrections
  curves
}

#' Require one nonmissing logical control
#' @noRd
.validate_scalar_logical <- function(x, name) {
  if (!is.logical(x) || length(x) != 1L || is.na(x)) {
    stop("'", name, "' must be one nonmissing logical value.")
  }
  invisible(x)
}
...
#' @noRd
.solve_semi_markov <- function(curves, structure, start_state, requested_times,
  ...
  if (!check_grid) {
    warning(
      "The semi-Markov result has not passed the production grid-convergence gate.",
      call. = FALSE
    )
  }
  if (tau == 0) {
    ...
    return(list(prob = one$prob, grid = 0, grid_step = NA_real_,
                converged = if (check_grid) TRUE else NA,
                error = if (check_grid) 0 else NA_real_, refinements = 0L,
  ...
  if (check_grid && intervals * 2L + 1L > max_grid_points) {
    stop("'max_grid_points' must permit at least one required grid refinement.")
  }
  ...
  for (refinement in seq_len(max_grid_refinements)) {
    ...
    last_error <- max(abs(
      fine_out[start_state, , , drop = FALSE] -
        coarse_out[start_state, , , drop = FALSE]
    ))
