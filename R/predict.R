...
#' @param times Finite nonnegative elapsed durations.
...
#'   state.
#' @param grid_step,target_grid_points,max_grid_points,grid_tol,max_grid_refinements,check_grid
#'   Numerical controls passed to the common semi-Markov solver.
...
#'   alias \code{P} have dimensions profile by starting state by occupied state
#'   by elapsed time. \code{state_occ} is the selected starting-state slice.
#'
...
predict.rfmstate <- function(object, newdata = NULL, times = NULL, s = 0,
  ...
  structure(
    list(
      ...
      support_by_origin = object$max_duration_by_origin,
