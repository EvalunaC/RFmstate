...
test_that("two-state exponential probabilities match analytic truth", {
  ...
  absorbing <- compute_trans_prob(
    list("A->B" = hazard_curve(0.4)), ms, times = times,
    start_state = "B", target_grid_points = 512,
    max_grid_points = 8193, grid_tol = 1e-5
  )
  expect_equal(unname(absorbing$entry_prob["B", "B", ]),
               rep(1, length(times)), tolerance = 1e-10)
})
...
test_that("competing absorbing destinations and probability invariants hold", {
  ...
  expect_equal(unname(result$entry_prob["A", , 1]), c(1, 0, 0),
               tolerance = 1e-10)
})
...
test_that("grid convergence is scoped to the requested starting state", {
  ms <- define_multistate(
    c("A", "B", "C"), "C", list(A = "B", B = "C")
  )
  curves <- list(
    "A->B" = data.frame(time = c(0, 3), hazard = c(0, 0.003)),
    "B->C" = data.frame(
      time = c(0, 0.3002, 3), hazard = c(0, 1, 1)
    )
  )

  result <- compute_trans_prob(
    curves, ms, times = c(0.3, 3), start_state = "A",
    target_grid_points = 16, max_grid_points = 257,
    grid_tol = 0.1, max_grid_refinements = 4
  )
  expect_identical(dimnames(result$entry_prob)[[1L]], "A")
  expect_true(result$grid_converged)
  expect_lte(result$grid_error, 0.1)

  expect_error(
    compute_trans_prob(
      curves, ms, times = c(0.3, 3), start_state = "B",
      target_grid_points = 16, max_grid_points = 257,
      grid_tol = 0.1, max_grid_refinements = 4
    ),
    "grid did not converge"
  )
})

test_that("zero-horizon identity calls briefly validate grid controls", {
  ms <- define_multistate(c("A", "B"), "B", list(A = "B"))
  hazards <- list("A->B" = hazard_curve(0.4, horizon = 1))

  expect_error(
    compute_trans_prob(hazards, ms, times = 0, target_grid_points = 1L),
    "target_grid_points"
  )
  expect_error(
    compute_trans_prob(hazards, ms, times = 0, max_grid_points = 1L),
    "max_grid_points"
  )
  expect_error(
    compute_trans_prob(hazards, ms, times = 0, max_grid_refinements = 0L),
    "max_grid_refinements"
  )
  expect_error(
    compute_trans_prob(hazards, ms, times = 0, grid_tol = 0),
    "grid_tol"
  )
  expect_error(
    compute_trans_prob(hazards, ms, times = 0, prob_tol = 0),
    "prob_tol"
  )
  expect_error(
    compute_trans_prob(hazards, ms, times = 0, grid_step = 0),
    "grid_step"
  )
})
